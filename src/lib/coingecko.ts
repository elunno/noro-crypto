...
import type { GlobalData, MarketCoin, MarketChart } from "@/types/coingecko";
...
export async function getTopCoins(limit = 10): Promise<MarketCoin[]> {
    ...
    const res = await fetch(`${COINGECKO_API}/coins/markets?${params.toString()}`, {
        next: { revalidate: 60 },
    });
 ...
 */
export async function getMarketChart(coinId = "bitcoin", days = 30): Promise<MarketChart> {
    const params = new URLSearchParams({
        vs_currency: "usd",
        days: String(days),
    });

    const res = await fetch(`${COINGECKO_API}/coins/${coinId}/market_chart?${params.toString()}`, {
        next: { revalidate: 60 },
    });
